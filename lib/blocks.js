...
var bitcore = require('rhombus-bitcore-lib');
...
BlockController.DEFAULT_BLOCK_CACHE_SIZE = 3000;
...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    witnessmerkleroot: blockObj.header.witnessMerkleRoot,
    tx: transactionIds,
    ...
    reward: this.getBlockReward(blockObj) / 1e8,
...
BlockController.prototype.getBlockReward = function(blockObj) {
  return 0;
  /*
...
BlockController.prototype.current = function(req, res) {
  var self = this;

  self.node.services.bitcoind.getBestBlockHash(function(err, hash) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
    self.node.services.bitcoind.getBlockOverview(hash, function(err, block) {
      if(err) {
        return self.common.handleErrors(err, res);
      }
      ...
      var data = {
          hash: block.hash,
          height: block.height,
          time: d.toISOString().substring(0,19)+'Z',
          tx: block.txids
        };
